         ...
         const char *x_szDebugPath)
{
    ...
    // pass 1
    {
        ...
        // Read library versions from .XBLD section if present
        m_Header.dwLibraryVersions = 0;
        for(uint32 s = 0; s < x_Exe->m_Header.m_sections; s++)
        {
            if(memcmp(x_Exe->m_SectionHeader[s].m_name, ".XBLD\0\0\0", 8) == 0)
            {
                uint32 xbld_size = x_Exe->m_SectionHeader[s].m_virtual_size;
                uint32 num_entries = xbld_size / sizeof(LibraryVersion);
                // Count non-empty entries
                LibraryVersion *entries = (LibraryVersion *)x_Exe->m_bzSection[s];
                for(uint32 e = 0; e < num_entries; e++)
                {
                    if(entries[e].szName[0] != '\0')
                        m_Header.dwLibraryVersions++;
                }
                printf("Xbe::Xbe: Found %d library versions in .XBLD section\n", m_Header.dwLibraryVersions);
                break;
            }
        }
        if(m_Header.dwLibraryVersions == 0)
            m_Header.dwLibraryVersions = 1; // fallback dummy entry
    ...
    // pass 3
    {
        ...
        *c = '\0';
        ...
        // Write library versions (from .XBLD section or placeholder)
        {
            m_LibraryVersion = new LibraryVersion[m_Header.dwLibraryVersions];

            // Try to read from .XBLD section
            bool found_xbld = false;
            for(uint32 s = 0; s < x_Exe->m_Header.m_sections; s++)
            {
                if(memcmp(x_Exe->m_SectionHeader[s].m_name, ".XBLD\0\0\0", 8) == 0)
                {
                    LibraryVersion *entries = (LibraryVersion *)x_Exe->m_bzSection[s];
                    uint32 num_entries = x_Exe->m_SectionHeader[s].m_virtual_size / sizeof(LibraryVersion);
                    uint32 v = 0;
                    for(uint32 e = 0; e < num_entries && v < m_Header.dwLibraryVersions; e++)
                    {
                        if(entries[e].szName[0] != '\0')
                        {
                            memcpy(&m_LibraryVersion[v], &entries[e], sizeof(LibraryVersion));

                            // Set kernel and XAPI library version addresses
                            if(memcmp(entries[e].szName, "XBOXKRNL", 8) == 0)
                                m_Header.dwKernelLibraryVersionAddr = hwc + v * sizeof(LibraryVersion);
                            else if(memcmp(entries[e].szName, "XAPILIB", 7) == 0 ||
                                    memcmp(entries[e].szName, "XAPILIBD", 8) == 0)
                                m_Header.dwXAPILibraryVersionAddr = hwc + v * sizeof(LibraryVersion);

                            printf("Xbe::Xbe: Library %d: %.8s %d.%d.%d\n", v,
                                   m_LibraryVersion[v].szName,
                                   m_LibraryVersion[v].wMajorVersion,
                                   m_LibraryVersion[v].wMinorVersion,
            ...
            if(!found_xbld)
            {
                ...
                char tmp[9];
                snprintf(tmp, sizeof(tmp), "CXBE0");
                for(uint32 c = 0; c < 8; ++c)
                    m_LibraryVersion[0].szName[c] = tmp[c];
                m_LibraryVersion[0].wMajorVersion = 0;
                m_LibraryVersion[0].wMinorVersion = 0;
                m_LibraryVersion[0].wBuildVersion = 0;
                m_LibraryVersion[0].dwFlags.QFEVersion = 0;
                m_LibraryVersion[0].dwFlags.Approved = 0;
                m_LibraryVersion[0].dwFlags.bDebugBuild = 0;
            }
