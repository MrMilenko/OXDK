...
/* The single definition of the shared video state declared in SDL_xboxvideo.h. */
...
/* Not const: SDL_ListModes hands this straight back as SDL_Rect**. */
...
{
	/* vid_modes is already an array of SDL_Rect*, so it decays to what the caller
	   wants. Taking its address gave a pointer to the array instead. */
	return vid_modes;
}
				...
				int width, int height, int bpp, Uint32 flags)
{
   ...
   D3DXCompileShader( g_strPixelShaderProgram,
	...
	IDirect3DVertexBuffer9_Lock(vertexBuffer, 0, 0, (void **)&pLockedVertexBuffer, 0L );	 
