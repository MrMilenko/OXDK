...
/* static, not bare __inline: MSVC emits an __inline function when it is needed, but
   under C99 an inline definition with external linkage provides no out-of-line copy,
   so the call was left undefined at link time. */
static __inline FLOAT ConvertThumbstickValue( SHORT sThumbstickValue, SHORT sDeadZone )
{
